...
all_ids = ['581450'] #['114823','157336','585256','581450','725751','avg']; #from Ribeiro 2023 Fig1
...
for ids in range(0,len(all_ids)):
    ...
    def train(sess, opt, kappa, beta1, beta2, beta3, y, b1, b2, b3, subject_id):
        sess.run(tf.global_variables_initializer())
        k = 30.0
        nIter = 1000  #1000
        ...
        for i in range(nIter):
            sess.run(opt, {kappa: k, beta1: b1, beta2: b2, beta3: b3})
            k = k - k*5/nIter #0.005
            #print(sess.run(reg2))
        ...
        yfinal = sess.run(y)
        return yfinal, reg1_all, reg2_all, reg3_all
    ...
    for i1 in range(0, numb1):
        for i2 in range(0, numb2):
            for i3 in range(0, numb3):
                ...
                print('running elastic net b1:' + str(b1) + ', b2:' + str(b2) + ', b3:' + str(b3))
                
                ## Minimal path length
                suffix = tgt + '_' + subject_id + '_b1_' + "%d"%(1e3*b1) + '_b2_' + "%d"%(1e3*b2) + '_b3_' + "%d"%(1e3*b3)
                opt = tf.train.RMSPropOptimizer(current_eta, momentum = m).minimize(yx_cost + beta1 * reg1 + beta2 * reg2 + beta3 * reg3, global_step = global_step)
                result = train(sess, opt, kappa, beta1, beta2, beta3, y, b1, b2, b3, subject_id)
