function im = getMouseAreasX(kmap_hor, kmap_vert, kmap_rad, smoothingFac, threshold)%, mask)
...
% imseg = (sign(VFS-threshSeg/2) - sign(-VFS-threshSeg/2) + 1)/2;  %threshold visual field sign map at +/-1.5sig
% id = find(imseg);
...
colorbar off
axis image
title(['4. +/' num2str(threshold) 'xSig = ' num2str(threshSeg)]) 
