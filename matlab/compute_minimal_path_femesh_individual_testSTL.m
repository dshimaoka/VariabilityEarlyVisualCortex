...
for sid= 2%:numel(subject_id)
    % %% import mask from export_geometry_test.py
    %load('array_3d.mat','array_3d','grid_x','grid_y');%,'mask');
    load(fullfile(loadDir, ['geometry_retinotopy_'  subject_id{sid}   '.mat']),'array_3d','grid_x','grid_y');%,'mask');
    ...
    for kk = 11
        ...
        switch kk
            case 1
                suffix = 'mlab_laplacian3';
            case 2
                suffix = 'mlab_laplacian6';
            case 3
                suffix = 'swks_decimate50';
            case 4
                suffix = 'w3b_simplify1';
            case 5
                suffix = 'w3b_simplify2';
            case 6
                suffix = 'w3b_smooth6';
            case 7
                suffix = 'w3b_smoothEdges';
            case 8
                suffix = 'w3b_smoothEdges_simplify6';
            case 9
                suffix = 'w3b_split';
            case 10
                suffix = 'mlab_simplify2';
            case 11
                suffix = 'mlab_hclaplacian';
            case 12
                suffix = 'mlab_hclaplacian_simplify';
        end
        model = createpde(1);
  ...
  %% show surface and shortest path distance
    ...
    trisurf(faces, vertices(:,1), vertices(:,2), vertices(:,3), 'FaceColor', 'c', 'EdgeColor', 'k');%,'facealpha', .05);
    shading interp
    light("Style","local","Position",[0 -80 0]);
    material dull
    hold on;
    % scatter3(vertices(find(mask), 1), vertices(find(mask), 2), vertices(find(mask), 3), 10, 'b', 'filled');
    scatter3(vertices(xy2node(yidx,xidx), 1), vertices(xy2node(yidx,xidx), 2), vertices(xy2node(yidx,xidx), 3), 300, 'r', 'filled');
    view(180, 0);
    title('surface from behind');
    axis ij equal tight off;
    
    linkaxes(ax(:));

     screen2png(fullfile(saveDir, ['surface_minimal_path_hmax' num2str(hmax) '_' subject_id{sid}]));


    %% sanity check 2
    % load('/home/daisuke/Documents/git/VariabilityEarlyVisualCortex/results/fieldSign_avg_smoothed_arealBorder.mat')
 load(['/home/daisuke/Documents/git/VariabilityEarlyVisualCortex/results/geometry_retinotopy_' subject_id{sid} '_arealBorder.mat'], ...
     'areaMatrix')
 ...
 screen2png(fullfile(saveDir,['minimal_path_' type '_hmax' num2str(hmax) '_serie_' subject_id{sid}]))
