function [signMapThreshold, im_final] = getVisualBorder_h(signMap,...
    threshold,structuringElementRadius,structuringElementRadius2, visualCortexBorder)
...
structuringElement2 = strel('disk',structuringElementRadius2);
  ...
  signMapThresholdO = abs(signMapThreshold);
    if debug; subplot(331); imagesc(signMapThresholdO); axis equal tight; end
...
if isempty(visualCortexBorder)
    signMapBinary = abs(signMapThresholdO);
    if debug; subplot(332); imagesc(signMapThresholdO); axis equal tight; end
    %Visual cortex border = Dilate(Open(Close(abs(Sthresh)))
    temp = imclose(signMapBinary,structuringElement2);
    if debug; subplot(333); imagesc(temp); axis equal tight; end
    temp2 = imopen(temp,structuringElement2);
    if debug; subplot(334); imagesc(temp2); axis equal tight; end
    ...
    if debug; subplot(335); imagesc(visualCortexBorder); axis equal tight; end
end
...
if debug; subplot(336); imagesc(border); axis equal tight; end
border = abs(border);
%border = imclose(border, structuringElement);
if debug; subplot(337); imagesc(border); axis equal tight; end
border = bwmorph(border,'skel',Inf);%'thin',Inf);
if debug; subplot(338); imagesc(border); axis equal tight; end
border = bwmorph(border,'spur',10);
border = bwmorph(border, 'clean');
if debug; subplot(339); imagesc(border); axis equal tight; end
