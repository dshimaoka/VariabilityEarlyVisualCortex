...
def neighborhood(roi2D):
    """
        returns list of index that are connected insited roi2D
    ...
    h,w = roi2D.shape;
    x = np.transpose(np.array(range(h*w)).reshape([w, h])) #2D
    lst = []
    for i in range(w):
        for j in range(h):
            neighbors = []
...
def initial_condition(areaIdx, x):
    """
    ...
    idx = np.random.randint(x.shape[0], size = len(areaIdx))
