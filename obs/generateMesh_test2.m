...
% % Create a rectangular patch of vertices on z=0 to ensure a closed mesh
% [z0_rect_x, z0_rect_y] = meshgrid(linspace(min(X(:)), max(X(:)), 10), ...
%                                    linspace(min(Y(:)), max(Y(:)), 10));
% z0_rect_vertices = [z0_rect_x(:), z0_rect_y(:), zGrid(1) * ones(numel(z0_rect_x), 1)];
...
% all_vertices = [vertices_surface; vertices_z0; z0_rect_vertices];
% all_faces = [faces_surface; 
%              size(vertices_surface, 1) + delaunay(xGrid(:), yGrid(:)); 
%              size(vertices_surface, 1) + size(vertices_z0, 1) + delaunay(z0_rect_x(:), z0_rect_y(:))];
